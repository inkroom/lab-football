...
//        }
        ...
        if (returnValue != null) {
            ...
            if (returnValue instanceof MessageDto) {
                MessageDto dto = (MessageDto) returnValue;
                //获取新的token
                dto.setToken((String) ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest().getSession().getAttribute(BaseStatic.KEY_SESSION_TOKEN));
            }
...
//        log.info("token只="+((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest().getSession().getAttribute(BaseStatic.KEY_SESSION_TOKEN));
