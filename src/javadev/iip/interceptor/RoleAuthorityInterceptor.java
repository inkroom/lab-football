	...
	public String intercept(ActionInvocation invocation) throws Exception {
		...
		if (action instanceof Anonymous) {
			...
			if(userService.ifRoleHasAuthority("0", actionURL)){
				...
				log.info("----------error:role:actionName:" + invocation.getProxy().getMethod());
			...
			if (user != null) {
				...
				if (role!=null) {
					log.debug("role    true      true      true      true      true      true      true      true      true      true      true      true      ");
				}
				
				if (userService.ifRoleHasAuthority(role.toString(), actionURL)) {
					log.debug("userService    true      true      true      true      true      true      true      true      true      true      true      true      ");
				}
				
				if (role != null&&userService.ifRoleHasAuthority(role, actionURL)) {
