 ...
 * 登录拦截器，用于阻止未登录用户访问系统
 */
public class IsFinishArtInterceptor implements Interceptor {
	...
	public String intercept(ActionInvocation invocation) throws Exception {
		...
		String status = isPassService.getAuditStatusArt(o_id);
		
		
		
		if (status.equals("1")) {
			ActionContext.getContext().getSession().put("isFinshMessage", "艺术报表已经审核通过,请在汇总数据下查看。");
			return "index";
		}
