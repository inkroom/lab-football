 ...
 * 登录拦截器，用于阻止未登录用户访问系统
 */
public class IsFinishInterceptor implements Interceptor {
	...
	public String intercept(ActionInvocation invocation) throws Exception {
		...
		String status = isPassService.getAuditStatusFive(o_id);
		
		if (status.equals("1")) {
			ActionContext.getContext().getSession().put("isFinshMessage", "五年报表已经审核通过。");
			return "index";
		}
