...
//                LiveWebSocket.sendMessage(dto.toString());
    ...
    public MessageDto imageUpload(MultipartFile file) {
        Integer sch = (Integer) request.getSession().getAttribute(BaseStatic.KEY_SESSION_VER_SCH_ID);
        if (sch == null) return new MessageDto(Result.NO_AUTHORITY);
