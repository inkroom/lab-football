...
//        return true;
    ...
    protected void checkFileSize(MultipartFile multipartFile, String type) throws MessageException {
        ...
        log.info("   原始size=" + multipartFile.getSize() + "    " + size);
    ...
    protected void checkFileType(MultipartFile multipartFile, String type) throws MessageException {
        ...
        log.info("  type= " + multipartFile.getName() + "   " + multipartFile.getOriginalFilename() + "   " + multipartFile.getOriginalFilename().lastIndexOf(".") + "  " +
                "  " + (multipartFile.getOriginalFilename().substring(multipartFile.getOriginalFilename().lastIndexOf(".") + 1)));
        log.info("原始type = " + (multipartFile.getOriginalFilename().substring(multipartFile.getOriginalFilename().lastIndexOf(".") + 1) + "   "));
        for (String allowType : allTypes) {
            log.info(" allow  type = " + allowType);
            if (allowType.equals(multipartFile.getOriginalFilename().substring(multipartFile.getOriginalFilename().lastIndexOf(".") + 1))) {
                return;
            }
        }
    ...
    protected String getFilePath(MultipartFile file, String type) {
        return directory + File.separator + System.currentTimeMillis() + "." + file.getOriginalFilename().substring(file.getOriginalFilename().lastIndexOf(".") + 1);
    }
