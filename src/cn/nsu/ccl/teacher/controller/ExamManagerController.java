...
public class ExamManagerController {
	...
	public void createExam(int libraryId ,String examName,
		...
		//创建一个考试信息的对象
		ExamInfo examInfoEntity = new ExamInfo();
		//添加老师邮箱属性信息
    ...
    boolean state = false;
      ...
      if (multipartResolver.isMultipart(request)) { 
          ...
          //接收从excel转回的list集合
  		ArrayList<StudentInfo> list = service.getExamService().excelToList(path);
  		for(int i = 0; i < list.size(); i++){
  			StudentInfo studentInfoEntity = list.get(i);
  			//调用service将数据保存到数据库
	...
	public String toEditExamInfo(){
		...
		List<ExamInfo> list = service.getExamService().getExamInfoByTeacherEmail(teacherEmail);
		//根据examid获取是否有考生信息
		//1.遍历考试信息
	...
	public void getQuestionLibListByExamId(int examId,HttpServletResponse response){
		...
		int libraryId = -1;
		//初始化一个题库列表信息，用于返回值
		QuestionLibList qEntity = new QuestionLibList();
		//新建一个JSONArray对象，用于存储返回的信息
		JSONArray jsonArray = new JSONArray();
		//获取在会话中的教师邮箱信息
		String teacherEmail = (String) session.getAttribute("teacherEmail");
		//获取该教师所有的考试信息
		List<ExamInfo> examInfoEntities = service.getExamService().getExamInfoByTeacherEmail(teacherEmail);
		//遍历所有的考试信息，匹配与传入的考试id相等的数据，并获取到对应的题库id
		for(int i = 0; i < examInfoEntities.size();i++){
			ExamInfo  examInfoEntity = examInfoEntities.get(i);
			if(examInfoEntity.getExamId()==examId){
				libraryId = examInfoEntity.getQuestionListNumber();
			}
		}
		//使用教师id获取对应的题库列表信息
		List<QuestionLibList> questionLibListEntities = service.getQuestionLibService().getQuestionLibListByTeacherEmail(teacherEmail);
		//遍历题库信息找到与之前的题库id相同的题库列表信息
		for(int j = 0; j < questionLibListEntities.size();j++){
			QuestionLibList questionLibListEntity = questionLibListEntities.get(j);
			//匹配与之前的题库id相同的数据
