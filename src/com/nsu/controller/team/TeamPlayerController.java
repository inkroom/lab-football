...
public class TeamPlayerController extends BaseController{
	...
	public String TeamPlayerView(@PathVariable("num")String num, HttpSession session, Model model){
		...
		return TeamVar.TEAM_PLAYER_PAGE;
	}
...
//		}
		...
		// 得到省级记录
		try {
			playerInfo.setProvenceHours(TeamVar.PojoToString(playerInfoService.getPlayerActivityInfo(A_ID, "1")));
		} catch (Exception e) {
			e.printStackTrace();
		...
		// 得到市级记录
		try {
			playerInfo.setCityHours(TeamVar.PojoToString(playerInfoService.getPlayerActivityInfo(A_ID, "2")));
		} catch (Exception e) {
			e.printStackTrace();
		...
		// 得到县级记录
		try {
			playerInfo.setCountryHonrs(TeamVar.PojoToString(playerInfoService.getPlayerActivityInfo(A_ID, "3")));
		} catch (Exception e) {
			e.printStackTrace();
		...
		// 得到校级记录
		try {
			playerInfo.setSchoolHonrs(TeamVar.PojoToString(playerInfoService.getPlayerActivityInfo(A_ID, "4")));
		} catch (Exception e) {
			e.printStackTrace();
		...
		// 得到其他记录
		try {
			playerInfo.setOtherHours(TeamVar.PojoToString(playerInfoService.getPlayerActivityInfo(A_ID, "5")));
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String teamAuditPlayerOperation(String pid, String operType, String ptNum,HttpSession session){
		...
		if(!V.checkEmpty(teamID)){
			if(!V.checkEmpty(pid) && !V.checkEmpty(operType)){
				...
				if(V.checkEmpty(result)){
					...
					String teamName = RequestUtil.getAccountInfoInSession(session, "aName");
