...
public class TeamMatchController extends BaseController {
	...
	public String teamScheduleArrange(@PathVariable("comID")String comID, @PathVariable("num") String num, Model model, HttpSession session){
		...
		Page page = null;
		int scheduleNum = 0;
		if(V.checkEmpty(comID) == false && V.checkEmpty(TEAM_ID) == false){
			//根据赛事ID查询赛程信息
		...
		return TeamUtil.TEAM_MATCH_PAGE_TWO;
		}else{
			// 登录回话失效
			log.info("session登录失效，没获取到球队id");
			return TeamUtil.TEAM_SESSION_OUTIME_PAGE;
	...
	@ResponseBody
	public String MatchView(String R_ID, HttpSession session){
		log.info("是否传送成功！"+R_ID);
        JSONObject jsonstr = new JSONObject();
        //创建JSONArray对象
        JSONArray jsonHomeList = new JSONArray();
        JSONArray jsonVisitList = new JSONArray();       
        JSONArray jsonHomeInfo = new JSONArray();
        JSONArray jsonVisitInfo = new JSONArray();
