...
public class UpdatePhoneController extends BaseController {
	...
	public void getEmail(HttpSession session, HttpServletResponse response) {
		try {
			...
			session.setAttribute("SMSPhone", InfoProtUtil.xorInfo(old_phone_sql));
			info.put("getPhone", InfoProtUtil.xorInfo(old_phone_sql).replaceAll("(\\d{3})\\d{4}(\\d{4})", "$1****$2"));
	...
	public String getPhoneCode(@RequestParam(value = "oldPhoneCode") String oldPhoneCode, @RequestParam(value = "type") String type, HttpSession session, HttpServletResponse response) {
		...
		try {
			...
			int A_ID = Integer.parseInt(map1.get("A_ID").toString());
			String old_phone_sql = iUpdatePhoneService.getPhoneById(A_ID);
			 if (jedisClient.get(type+"web"+InfoProtUtil.xorInfo(old_phone_sql).toString()+"num") != null){
		            if (Integer.parseInt(jedisClient.get(type+"web"+InfoProtUtil.xorInfo(old_phone_sql).toString()+"num"))>5){
		            	info.put("error","验证码错误");
		                log.info(" if *** \"mob\"+mobile+\"num\" = **** = " + jedisClient.get(type + "web" + InfoProtUtil.xorInfo(old_phone_sql).toString() + "num"));
		                jedisClient.del(type+"web"+InfoProtUtil.xorInfo(old_phone_sql).toString());
		                jedisClient.del(type + "web" + InfoProtUtil.xorInfo(old_phone_sql).toString() + "num");
		                return info.toString();
		            }
		        }
		        if (oldPhoneCode.equals(jedisClient.get(type+"web"+InfoProtUtil.xorInfo(old_phone_sql).toString()))) {
		        	info.put("error","1");
		        } else {
		        	info.put("error","验证码错误");
		            if (jedisClient.get(type+"web"+InfoProtUtil.xorInfo(old_phone_sql).toString()+"num") != null){
		                jedisClient.incr(type + "web" + InfoProtUtil.xorInfo(old_phone_sql).toString() + "num");
		            }
		        }
		        return info.toString();
		} catch (Exception e) {
	...
	public String checkOldCode(@RequestParam(value = "new_phone") String new_phone,
		...
		try {
			if (!phoneCheck(new_phone)) {
				...
				map.put("PHONE", InfoProtUtil.xorInfo(new_phone));
				map.put("A_ID", A_ID);
