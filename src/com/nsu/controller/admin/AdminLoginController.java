...
public class AdminLoginController extends BaseController {
    ...
    public String loginView(HttpSession session){
        ...
        try {
            ...
            String temp = map.get(CommonVar.Account.TYPE).toString();
            if (temp.equals("0")){
                ...
                if (userName == null || userName.equals("")){
                   ...
                   if (password == null || password.equals("")){
                       ...
                       if (code.equals(session.getAttribute(Constants.RANDOM_CODE))){
                           //获取数据中的user
                           UserBean userBean = adminService.getUserByUsername(InfoProUtil.xorInfo(userName));
                           if (userBean != null){
                               if (InfoProUtil.comparePass(userBean.getPassword(),session.getAttribute(Constants.SALT_IN_SESSION).toString(),password)){
                                   ajaxBean.setSuccess(true);
                                   ajaxBean.setMsg(LOGIN_SUCCESS);
                                   ajaxBean.setStatus("200");
                                   Map<String,Object> userMap = new HashMap<String,Object>();
                                   userMap.put(CommonVar.Account.TYPE, userBean.getType());
                                   userMap.put(CommonVar.Account.USERNAME,userBean.getUserName());
                                   userMap.put(CommonVar.Account.ID,userBean.getId());
