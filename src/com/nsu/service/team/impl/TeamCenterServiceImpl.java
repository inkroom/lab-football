...
//            log.info("查看获胜场数！！！！！"+win);
					...
					if(V.checkEmpty(aid)){
						...
						try{
							...
							if(teamMap!=null && teamMap.size()>0){
								int num = TeamUtil.calculaPlayerScore(teamMap);
	...
	public String updateTeamBasicData(TeamBasicInfoBean teamBasicInfo) {
		...
		try {
			...
			String account = InfoProtUtil.xorInfo(teamBasicInfo.getAccount());
	...
	@Override
	public String updateTeamPhotoes(String teamID, String filePath, String type) {
	...
	public String findEamilVerificationCode(String aid) {
		try {
			return teamSendVerificationCodeDao.findEamilVerificationCode(aid);
		} catch (Exception e) {
	 ...
	 */
	@Override
	public int findAIDBYPIDService(String P_ID) {
		try {
	 ...
	 */
	@Override
	public int findAIDBYCOACHIDService(String COACH_ID) {
		try {
	...
	@Override
	public int findAIDBYORGService(String ORG) {
		try {
