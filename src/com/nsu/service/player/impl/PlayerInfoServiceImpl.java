...
public class PlayerInfoServiceImpl implements PlayerInfoService {
	...
	public Map<String, Object> getPlayerInfo(String id) throws Exception {
		...
		//得到学籍号
		map.put("STU_NUM", playerInfoDao.getStuNum(map.get("A_ID_CARD").toString()));
		
		// 得到信息，将敏感信息解密后再存放
		String A_ID_CARD = InfoProtUtil.xorInfo(map.get("A_ID_CARD").toString());
		map.put("A_ID_CARD", A_ID_CARD);
	...
	public boolean updateWonderfulPhoto(String a_id, String filePath) throws Exception {
		...
		int result = playerInfoDao.insertWonderfulPhoto(wonder);
		if (result == 1)
			return true;
		return false;
	}
