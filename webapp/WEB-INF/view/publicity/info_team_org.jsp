...
<script type="text/javascript">
	function find() {
		...
		if(search==null || search.length==0){
			...
			layer.msg("球队名只能包含中英文和数字！");
			$("#search").val("");
		}else if(search.length>40){
			layer.msg("球队名最大长度为40！");
			$("#search").val("");
		}else{
			searchList(search);
		}
	...
	function searchList(search) {
		$.ajax({
			...
			type : "POST",
			dataType : 'json',
			url : "${pageContext.request.contextPath }/info/searchTeam.html",
			...
			success : function(result) {
				if (result.success) {
					if(result.size==0){
						...
						$("#addjson").empty();
						$("#page1").empty();
						$.each(result.teamAll,function(index,item){
							var _tr=$("<tr>"+
								"<td>"+result.teamAll[index].teamName+"</td>"+
								"<td>"+result.teamAll[index].leaderName+"</td>"+
								"<td>"+result.teamAll[index].organization+"</td>"+
								"<td>"+result.teamAll[index].createDate+"</td>"+
								"<td><button class='btn btn-success-outline radius' onClick='modaldemo("+result.teamAll[index].teamNum+")'>查看详情</button></td>"+
							"</tr>")
				        	$("#addjson").append(_tr);
						})
					}
...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 机构管理 <span class="c-gray en">&gt;</span> 球队公示 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href='${base_path }/info/teamInfoOrg.html');" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="">
    <div class="cl pd-5 bg-1 bk-gray mt-20">
    <a class="text-r f-r" style="text-decoration: none;">
        <input type="text" class="input-text" style="width:350px" placeholder="请输入球队名" id="search" name="search">
    ...
    <div class="modal-dialog">
        <div class="modal-content radius">
            ...
            <div class="modal-body text-c">
                <table id="teamDetail" class="table table-border table-bordered table-bg radius table-hover ">
                    <tbody class="text-c">
                    
                    </tbody>
                </table>
                
                <table id="teamTable" class="table table-border table-bordered table-bg radius mt-20">
                    <tbody class="text-c">
...
<script>
    function modaldemo(num) {
		$.ajax({
			...
			type : "POST",
			dataType : 'json',
			url : "${pageContext.request.contextPath }/info/teamInfoDetail.html",

			success : function(result) {
				if (result.success) {
					$("#modal-demo").modal("show");
					
					$('#teamDetail').children('tbody').empty();
					$('#teamTable').children('tbody').empty();
					var str = "<tr class='text-c'> <th>球队</th> <th>领队</th> <th>所属机构</th></tr>"+
						"<tr><td>" + result.detail.teamName + "</td>"+
				    	"<td>" + result.detail.leaderName + "</td>"+
				    	"<td>" + result.detail.organization + "</td>"+
				    	"</tr>";
				    str += "<tr class='text-c'> <th>球队类型</th> <th>球队等级</th> <th>创立时间</th></tr>"+
				    	"<tr><td>" + result.detail.type + "</td>"+
				    	"<td>" + result.detail.level + "</td>"+
				    	"<td>" + result.detail.createDate + "</td></tr>";
				    str += "<tr class='text-c'> <th colspan='4'>励志标语</th> </tr>"+
				    	"<tr><td colspan='4'>" + result.detail.logo + "</td></tr>";
					$('#teamDetail').children('tbody').append(str);
					
					var str2 = "<tr><th colspan='3'>球队组成</th></tr>"+
						"<tr><th colspan='3'>教练员</th></tr>"+
						"<tr><td>姓名</td><td>性别</td><td>年龄</td></tr>";
					$.each(result.coach,function(index,item){
						str2+="<tr><td>"+result.coach[index].name+"</td><td>"+result.coach[index].sex+"</td><td>"+result.coach[index].birthday+"</td></tr>"
					})
					
	                str2 += "<tr><th colspan='3'>球员</th></tr>"+
	                    "<tr><td>姓名</td><td>性别</td><td>年龄</td></tr>";
                    $.each(result.player,function(index,item){
						str2+="<tr><td>"+result.player[index].name+"</td><td>"+result.player[index].sex+"</td><td>"+result.player[index].age+"</td></tr>"
					})
	...
	$.ajax({
		...
		success:function(data){
			...
			$.each(json.pageInfo.list,function(index,item){
				var _tr=$("<tr>"+
					"<td>"+json.pageInfo.list[index].teamName+"</td>"+
					"<td>"+json.pageInfo.list[index].leaderName+"</td>"+
					"<td>"+json.pageInfo.list[index].organization+"</td>"+
					"<td>"+json.pageInfo.list[index].createDate+"</td>"+
					"<td><button class='btn btn-success-outline radius' onClick='modaldemo("+json.pageInfo.list[index].teamNum+")'>查看详情</button></td>"+
				"</tr>")
	        	$("#addjson").append(_tr);
			})
			 //此处仅仅是为了演示变化的内容
