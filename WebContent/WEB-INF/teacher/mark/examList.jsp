<%@page import="cn.nsu.ccl.teacher.entity.ExamInfo"%>
...
<!-- 查看考试信息列表 -->
            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>查看成绩 <small>--请选择考试信息</small></h5>
                    </div>
                    <div class="ibox-content">
                        ...
                        <table class="table table-striped table-bordered table-hover dataTables-example">
                            ...
                            <tbody>
                            <%List<ExamInfo> list = (List<ExamInfo>)request.getAttribute("examInfoList");
                            ...
                            %>
                                <tr class="gradeX">
                                    ...
                                    <td>${exam.endTime }</td>
                                    <td>
                                    <span id="notClick<%=examInfoEntity.getExamId() %>" style="display:none">暂未完成考试</span>
                                    <a href="javascript:void(0);"  onclick="downloadMark('<%=examInfoEntity.getExamId()%>')" style="display:none" id="click<%=examInfoEntity.getExamId() %>">导出考试成绩</a>
                                    </td>
                                </tr>
                                <%} %>
                            </tbody>
    ...
    <script>
        ...
        //动态设置导出考试成绩标签
        var readJson = JSON.parse('<%=request.getAttribute("examState")%>');
        console.log(readJson);
    	for(var p in readJson){
    		//如果有考生信息，就隐藏到带点击事件的标签，显示不带点击事件的标签
    		if(readJson[p].state==="notEnd"){
    			console.log("notEnd");
    			//显示不带点击事件的标签
    			//找到带点击事件的id并隐藏
    		...
    		if(readJson[p].state==="end"){
    			console.log("end");
    			//显示带点击事件的标签
    			//找到不带点击时间的id并隐藏
    			var notClick = "notClick"+readJson[p].examId;
    			console.log("id="+notClick);
    	...
    	//导出考试成绩
		function downloadMark(examId){
			window.open('teacherDownloadMark?examId='+examId);
		}
    	</script>
