...
<script type="text/javascript">
	...
	function update_person(news){
		if(flag){
			flag = false;
			window.location.href = '${basePath }/school_admin/update_news_view_04.action?scId='+news.name+"&token="+'${token}';	
		}
	}
	function delete_person(news){
		if(flag){
			...
			layer.confirm('是否确认删除该条教育新闻', {
				...
				$.ajax({
					type: "post",
					url: "${basePath}/school_admin/delele_news_04",
						...
						if(result.success){
							layer.alert("操作成功", {
							    ...
							    ,closeBtn: 0
						  		...
						  		window.location.href = '${basePath }/school_admin/get_news_list_04.action?page=${page}&token='+result.token;
						  	});
					...
					error: function(data){
						layer.alert(result.errorInfo, {
						    ...
						    ,closeBtn: 0
					  		...
					  		window.location.href = '${basePath }/school_admin/get_news_list_04.action?page=${page}&token=${token}';
					  	});
					}
				});
			});
