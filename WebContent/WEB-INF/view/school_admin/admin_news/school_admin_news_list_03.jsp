				...
				<table class="table table-hover table-bordered text-center" id="myTab">
					<thead>
						<tr>
							...
							<td colspan="2">操时&nbsp&nbsp&nbsp&nbsp作</td>
						</tr>
					</thead>
					<tbody id="tbody">
...
<script type="text/javascript">
	...
	function update_person(news){
		if(flag){
			flag = false;
			window.location.href = '${basePath }/school_admin/update_news_view_03.action?scId='+news.name+"&token="+'${token}';	
		}
	}
	function delete_person(news){
		if(flag){
			flag = false;
			layer.confirm('是否确认删除该条教育新闻', {
				  btn: ['是','否'] //按钮
			}, function(){
				layer.msg('请稍后...',{
					icon:16
				...
				$.ajax({
					type: "post",
					url: "${basePath}/school_admin/delele_news_03",
						...
						if(result.success){
							layer.alert("操作成功", {
							    ...
							    ,closeBtn: 0
						  		...
						  		window.location.href = '${basePath }/school_admin/get_news_list_03.action?page=${page}&token='+result.token;
						  	});
					...
					error: function(data){
						layer.alert(result.errorInfo, {
						    ...
						    ,closeBtn: 0
					  		...
					  		window.location.href = '${basePath }/school_admin/get_news_list_03.action?page=${page}&token=${token}';
					  	});
					}
				});
			});
