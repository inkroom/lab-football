...
<script type="text/javascript">
	...
	function update_person(news){
		if(flag){
			flag = false;
			window.location.href = '${basePath }/school_admin/update_news_view_07.action?scId='+news.name+"&token="+'${token}';	
		}
	}
	function delete_person(news){
		if(flag){
			flag = false;
			layer.confirm('是否确认删除该条自我拓展', {
				  btn: ['是','否'] //按钮
			}, function(){
				layer.msg('请稍后...',{
					icon:16
				...
				$.ajax({
					type: "post",
					url: "${basePath}/school_admin/delele_news_07",
						...
						if(result.success){
							layer.alert("操作成功", {
							    ...
							    ,closeBtn: 0
						  		...
						  		window.location.href = '${basePath }/school_admin/get_news_list_07.action?page=${page}&token='+result.token;
						  	});
					...
					error: function(data){
						layer.alert(result.errorInfo, {
						    ...
						    ,closeBtn: 0
					  		...
					  		window.location.href = '${basePath }/school_admin/get_news_list_07.action?page=${page}&token=${token}';
					  	});
					}
				});
			});
